...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    required init?(coder aDecoder: NSCoder) {
        do {
            let dictionary = try PlistConverter.getDictionaryFromFile("VendingInventory", ofType: "plist")
    ...
    @IBAction func purchase() {
        if let currentSelection = currentSelection {
            do {
                ...
                showAlert("Insufficient Funds", message: "Additional $\(amount) needed to complete transaction")
            } catch let error {
                fatalError("\(error)")
            }
        } else {
    ...
    func showAlert(_ title: String, message: String? = nil, style: UIAlertControllerStyle = .alert) {
        ...
        let okayAction = UIAlertAction(title: "Okay", style: .default, handler: dismissAlert)
    ...
    func dismissAlert(_ sending: UIAlertAction) {
        reset()
    }
